...
uint8_t Decode_Sat_Packet(uint8_t *packet, SPI_HandleTypeDef *hspi, UART_HandleTypeDef *huart, SPI_HandleTypeDef *fram_hspi, I2C_HandleTypeDef *hi2c)
{
	...
	switch (command)
	{
		...
		case (REQ_SCI_DATA):
			...
			HAL_UART_Transmit(huart, (uint8_t *) msg1, sizeof(msg1), 1);
			// Handle the request here
			Handle_ReqSciData_Packet(packet, hspi, hi2c, huart, fram_hspi);
			break;
		case (REQ_SCI_DATA2):
		...
		case (KILL):
			...
			HAL_UART_Transmit(huart, (uint8_t *) msg1, sizeof(msg1), 1);
			// Handle the request here
			Handle_Kill_Packet(packet, hspi, huart, fram_hspi);
			break;
		default:
			fprintf(stderr, "Invalid packet command received\n");
			return FAIL; // Packet cold not be decoded	
...
/**
  ...
  * @retval None
  */
void Handle_Kill_Packet(uint8_t *packet, SPI_HandleTypeDef *hspi, UART_HandleTypeDef *huart, SPI_HandleTypeDef *fram_hspi)
{
	// Create an Ack
	...
	SPI_FRAM_Read(fram_hspi, SPI_FRAM_TIME_ADDR, currTime, 8, huart, 0);
	double time = bytes_to_double(currTime);
	
	// Send an Ack
	Create_Acknowledgement(ackPacket, 0, time); 
...
void Handle_LogSci_Packet(uint8_t *packet, SPI_HandleTypeDef *hspi, UART_HandleTypeDef *huart, SPI_HandleTypeDef *fram_hspi)
{
	...
	if ((packet[0] & 0x04) == 0x04)// Only log science events that happen in the future
	{
		...
		HAL_UART_Transmit(huart, (uint8_t *) msg1, sizeof(msg1), 10);
		...
		// Send an Ack
		Create_Acknowledgement(ackPacket, 0, time); // The double value should be the current time
...
/**
  ...
  * @retval None
  */
void Handle_ReqSciData_Packet(uint8_t *packet, SPI_HandleTypeDef *hspi, I2C_HandleTypeDef *hi2c, UART_HandleTypeDef *huart, SPI_HandleTypeDef *fram_hspi)
{
...
//	HAL_UART_Transmit(huart, (uint8_t *) msg2, sizeof(msg2), 1);
	...
	for (int16_t i = 0; i < chunk; i++) 
	{
		...
		{
			snprintf(msg2, 100, "\nFRAM_SEARCH_ENDED: %d\n", i);
			HAL_UART_Transmit(huart, (uint8_t *) msg2, sizeof(msg2), 10);
			break;
		}
	...
	Create_ScienceData(sciDataPacket, sdp, 3, time);
...
void Handle_ReqLoc_Packet(uint8_t *packet, SPI_HandleTypeDef *hspi, UART_HandleTypeDef *huart, SPI_HandleTypeDef *fram_hspi)
{
	...
	Create_LocationData(locationPacket, latitude, longitude, altitude, time);
